...
@export var switches: Array[Marker2D] = []
...
func send_geometry() -> void:
	var message := {
		...
		"switches": switches.map(_marker_to_scaled_points)
	}
...
func _marker_to_scaled_points(marker: Marker2D) -> Array:
	var world_point := marker.global_position * world_scale
