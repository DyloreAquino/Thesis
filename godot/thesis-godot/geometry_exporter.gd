...
func send_geometry() -> void:
	var message := {
		"cmd": "setup_geometry",
		"walkable_area": _polygon_to_world_points(walkable_area),
		"entry_areas": entry_areas.map(_polygon_to_world_points),
		"exit_areas": exit_areas.map(_polygon_to_world_points),
	}
	sim_client.send_message(JSON.stringify(message))

func _polygon_to_world_points(poly: Polygon2D) -> Array:
	var points := []
	for local_point in poly.polygon:
		var world_point: Vector2 = poly.to_global(local_point)
