...
def spawn_random_agent(sim: jps.Simulation, entry_areas: list[Polygon], exit_journeys: list[tuple[int, int]]) -> bool:
    ...
    for _ in range(SPAWN_POSITION_ATTEMPTS):
        ...
        journey_id, exit_stage_id = random.choice(exit_journeys)
        desired_speed = max(
